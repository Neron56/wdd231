...
function createCourseList(filteredCourses) {
    ...
    filteredCourses.forEach(course => {
        ...
        cardContent.textContent = `${course.subject} ${course.number}`;
        ...
        document.querySelector("#courseList").appendChild(card);
    });
    let totalCredits = filteredCourses
        .map(course => course.credits)
        .reduce((sum, credits) => sum + credits, 0)
    document.querySelector("#creditCount").textContent = `The total number of credits from the courses listed above is ${totalCredits}`;
